foreign-person = Добавлять имущество может только казначей и его команда, уходи!

add-equipment-title = Добавление имущества в базу
welcome-text = Заполните форму для добавления нового имущества

select-category-prompt = Выберите категорию для имущества
select-category-btn = Выбрать категорию
create-new-category-btn = Создать новую категорию

select-place-prompt = Выберите место хранения
select-place-btn = Выбрать место

category-name-prompt = Введите название новой категории
category-created = Категория "{ $name }" успешно создана!
category-selected = Выбрана категория: { $name }
category-error = Ошибка при создании категории
category-label = Категория
category-invalid = Некорректная категория

place-selected = Выбрано место: { $name }
place-label = Место хранения
place-invalid = Некорректное место хранения

field-input-prompt = Введите значение для: { $field }
field-empty-error = Поле не может быть пустым

item-saved = Имущество "{ $name }" успешно добавлено!
item-error = Ошибка при добавлении имущества

back-button = Назад
submit-item = Сохранить имущество
cancel-button = Отмена
skip-button = Пропустить

required-hint = обязательные поля
required-fields = Обязательные поля отмечены *

invalid-number = Введите корректное число больше 0
invalid-category = Сначала выберите категорию
invalid-place = Сначала выберите место хранения
invalid-data = Данные заполнены некорректно

database-error = Ошибка подключения к базе данных

field-not-selected = Сначала выберите поле
schema-not-found = Схема не найдена
empty-value = —